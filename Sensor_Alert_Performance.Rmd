...
This will give us an overview of the low-cost(Purple Air) sensors networks across India & specifically over Indo-Gangetic Plain. We can look into the AQ-Particluate matter of 2.5 microns (PM2.5) data based on the solutions (projects) like SAMOSA, NASA, UTN or by sensor names such as NASA_AQCS_102, UTN102 or SAMOSA_0001. We can further filter based on city, district or state. We can find the number of sensors under each project & also easy to download in .csv, .xlsx or .pdf file formats. 
...
Here we can explore & analyse the time series pattern of a sensor or multiple sensors for last 30 days. The time series measures PM2.5 concentrations of both channels A & B. Scatter plot helps us to know how each channel of the sensors are performing. We can compare against sensors, distcirts or states. In QA-QC, check box 0 is to see noisy data and 1 to see data after performing basic QA-QC. This will help us to diagnose the non-perfroming channel. 
