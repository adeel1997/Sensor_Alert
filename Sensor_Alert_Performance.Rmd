...
leaflet(pas_updated) %>%
    ...
    addLegend(labels = labels,colors =colores , "topright",title = "Air Quality Index ")
...
for (i in 1:length(a)) {
    ...
    try ({
        ...
        Sensor_daily$lat <- as.list(pas %>% pas_filter(label==a[i]) %>% select(round(latitude, 3)))$latitude
        Sensor_daily$long <- as.list(pas %>% pas_filter(label==a[i]) %>% select(round(longitude,3)))$longitude
