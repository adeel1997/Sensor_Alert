---
title: | 
    ![](/home/jovyan/Sor Alert /Files/PA_logo.png){width=1in} 
    Sensor Alert and Performance
    ![](/home/jovyan/Sensor Alert /Files/CEEW_logo.jpg){width=2in} 
...
initializeMazamaSpatialUtils("/home/jovyan/Sensor Alert /Spatial_data")
## Creating the pas object

pas <- pas_createNew(countryCodes = "IN",baseUrl = 'https://www.purpleair.com/json?all=true', lookbackDays = 30,includePWFSL = FALSE)
...
## Selecting Project based Sensors and selecting limited columns

pas_subset <- pas %>% pas_filter(grepl('NASA',label)|grepl('UT',label),!grepl("B",label))%>%select(Sensor_Name,lastSeen,City,District,State,pm25_1hr,pm25_1day,pm25_1week)

## Visualizing the table (Fix header, Column and Button)

DT::datatable(pas_subset,extensions = c('Buttons','FixedHeader','FixedColumns'),
              options = list(autoWidth = T,fixedHeader = T,dom = 'Bfrtip',
...
## Adding colors according to India AQI scale
...
## Updating the pas object and converting temperature to Celsius and just selecting label on Sensor A to remove duplication

pas_updated <- pas %>% pas_filter(!grepl("B",label))%>%
    mutate(TempC= (temperature-32)*0.55)

## Now Plotting the pas object by giving proper pop up and label and adding popup information and other relevant information
...
"temperature = ", round(pas_updated$TempC, 0), "°C<br/>", 
"humidity = ", round(pas_updated$humidity, 0), "%<br/>",
"last Seen = ", pas$lastSeen,"<br/>",
"pm25_1hr =  ", round(pas_updated$pm25_1hr, 1), " µg/m3<br/>",
"pm25_1day = ", round(pas_updated$pm25_1day, 1), " µg/m3<br/>",
"pm25_1week = ", round(pas_updated$pm25_1week, 1), " µg/m3<br/>"))%>%
...
Inactive_Sensor <- pas %>% filter(!grepl("B",label)) %>% filter(lastSeen <Sys.Date()-1)%>% select(Sensor_Name,lastSeen,City,District,State,latitude,longitude) %>%
arrange(lastSeen)
...
list(extend="excel",text = "XLSX", title=paste0("Inactive_Sensors","_",Sys.Date())),
list(extend="pdf",text = "PDF",title=paste0("Inactive_Sensors","_",Sys.Date()))),
scrollX = F,columnDefs = list(list(width = '130px', targets = c(1,3,4,5)))),
filter = list(position = 'top', clear = T)
)
...
end_date <- Sys.Date()
## Initializing the Sensor Compiled
Sensor_Compiled <- c()

### Writing a loop to get all our sensors data, process it and save it in the Sensor Compiled
i <- 1
for (i in 1:length(a)) {
    ## Try is used to continue the loop even if there are errors 
    try ({
        ...
        Sensor_daily$District <- as.list(pas %>% pas_filter(label==a[i]) %>% select(District))$District
				...
				## This adds the Sensor by Sensor to one big dataframe which we call it by Sensor Compiled
...
## Removing missing value, adding performance indicators and sorting it based on RMSE
...
## Adding Average of PM2.5 from Channel A and Channel B

Sensor_Compiled <- Sensor_Compiled%>%
    mutate(PM25= rowMeans(cbind(PM25_A, PM25_B), na.rm=T))

## Highlight key is used so that filters can be used in Plotly 

tx <- highlight_key(Sensor_Compiled)

## Giving X-axis title and tick
axis_x <- list(
...
## Creating Widgets like Filters (Sensor, District and State)
## Width is for arranging it according to Page (12 is full width)
...
## Creating the plotly graphic (bscols) is a function from crosstalk package to arrange plotly

bscols(
    widths = c(6,6,10), 
    plot_ly(tx, x = ~Date, y = ~PM25, showlegend = FALSE) %>% 
        add_lines(color = ~Sensor_Name) %>%
        layout(xaxis=axis_x,
               yaxis=list(title="PM2.5 (µg/m³)"),title="Time Series"),
    plot_ly(tx, x = ~PM25_A, y = ~PM25_B, showlegend = FALSE) %>% 
        add_markers(color = ~Sensor_Name)%>%
...
## Xform is done to sort the x-axis (This is also done if we select legend it can select the District in that State)
...
This will give us an overview of the low-cost(Purple Air) sensors networks across India & specificlaly over Indo-Gangetic Plain. We can look the air quality data based on the solutions or projects like NASA, UTN, SAMOSA or by sensor names. We can further filter based on city, district or state. We can find the number of sensors under each project & can download in .csv,.xlsx or.pdf format. 
...
This gives us the list of sensor units that are inactive for more than 24 hours. 
...
#### **Exploratory Viz **
Time series and scatter plot among two channels of the sensor units.


#### **Box Plot **

This part allows to compare the sensor units in districts and states through [Box plot](https://towardsdatascience.com/understanding-boxplots-5e2df7bcbd51) . Double click on the State legend to see the box plot for a specific state. Also, we can add different states by clicking on State legend. 
